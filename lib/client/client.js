...
Client = exports.Client = function(options) {
  ...
  this._handleRequest = function(self, cb) {
    async function handle(self, options, cb) {
      var options = Object.assign({}, options);
      await self._request(options, function(err, response, result) {
        const retryAfter = response.headers["retry-after"];
        if (retryAfter) {
          console.log("Waiting " + retryAfter + "s to retry");
          const delay = t => new Promise(resolve => setTimeout(resolve, t));
          return delay(retryAfter * 1000).then(() => handle(self, options, cb));
        }
        return new Promise(resolve =>
          requestCallback(this, err, response, result, cb)
        );
      });
    }
    return new Promise(resolve => handle(self, self.options, cb));
  };

  this._buildOptions = function(method, uri, args) {
    var options,
      url,
      self = this,
      res,
      body =
        ...
        args.pop(),
      auth = this.options.get("password")
        ? ":" + this.options.get("password")
        : "/token:" + this.options.get("token"),
      encoded = new Buffer(this.options.get("username") + auth).toString(
        "base64"
      ),
    ...
    if (useOAuth) {
      // token is an oauth token obtained from OAuth2
      self.options.headers.Authorization = "Bearer " + token;
    } else {
      // token is an API token obtained from the Zendesk Settings UI
    ...
    if ("GET" === method) {
      ...
      "GET" !== method &&
      "application/json" === self.options.headers["Content-Type"]
    ) {
      self.options.body = "{}";
    }
...
Client.prototype.request = function(method, uri) {
  ...
  this._buildOptions(method, uri, args);

  self.emit("debug::request", self.options);

  return this._handleRequest(self, callback);
};
      ...
      if (record.hasOwnProperty(field) && response.hasOwnProperty(dataset)) {
        ...
        if (map[i].hasOwnProperty("all") && map[i].all === true) {
          ...
          if (map[i].hasOwnProperty("array") && map[i].array) {
            record[name] = findAllRecordsById(
              response[dataset],
              key,
              record[field]
            );
          } else {
            record[name] = findOneRecordById(
              response[dataset],
              key,
              record[field]
            );
...
function assembleUrl(self, uri) {
  ...
  if ("object" === typeof uri && Array.isArray(uri)) {
    ...
    return (
      self.options.get("remoteUri") + "/" + uri.join("/") + ".json" + params
    );
  } else if (
    "string" === typeof uri &&
    uri.indexOf(self.options.get("remoteUri")) === -1
  ) {
